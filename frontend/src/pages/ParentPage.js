...
const ParentPage = () => {
  ...
  // Fetch available slots on mount (so they’re available in any tab)
  useEffect(() => {
  ...
  const handleEditChildSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/parents/children/${editChildData.id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
 ...
 /* Render Home Tab: Fully Enhanced Dashboard Style with Distinct Sections */
const renderHome = () => {
  const now = new Date();
  ...
  const appointmentsToday = upcoming.filter(
    (app) => new Date(app.appointment_time).setHours(0, 0, 0, 0) === today
  ).length;
  const appointmentsThisWeek = upcoming.filter(
    (app) => new Date(app.appointment_time) <= new Date(today + 7 * 24 * 60 * 60 * 1000)
  ).length;
  ...
  const availableTherapists = availableSlots
    .map((group) => ({
      ...
      nextSlot: group.appointments
        .filter((slot) => new Date(`${slot.date}T${slot.start_time}`) >= now)
        .sort((a, b) => new Date(`${a.date}T${a.start_time}`) - new Date(`${b.date}T${b.start_time}`))[0],
    }))
    .filter((therapist) => therapist.nextSlot)
    .slice(0, 4);
    ...
    <div className="home-tab">
      ...
      <section className="stats-section">
        <div className="stats-container">
          <div className="stat-card">
            <h4>Today</h4>
            <p>
              {appointmentsToday} {appointmentsToday === 1 ? "Appointment" : "Appointments"}
            </p>
          </div>
          <div className="stat-card">
            <h4>This Week</h4>
            <p>
              {appointmentsThisWeek} {appointmentsThisWeek === 1 ? "Appointment" : "Appointments"}
            </p>
          </div>
          <div className="stat-card">
            <h4>Children</h4>
            ...
            {availableTherapists.map((therapist) => (
              <div key={therapist.id} className="therapist-card">
                ...
                <p>
                  ...
                  {new Date(`${therapist.nextSlot.date}T${therapist.nextSlot.start_time}`).toLocaleString([], {
                    weekday: "short",
                    month: "short",
                    day: "numeric",
                    hour: "2-digit",
        ...
        <div className="upcoming-timeline">
          {upcoming.slice(0, 5).map((app) => (
            <div key={app.id} className="timeline-card">
              <p>{new Date(app.appointment_time).toLocaleDateString("en-US", { weekday: "short", day: "numeric" })}</p>
              <p>{new Date(app.appointment_time).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p>
              <p>{app.child_name}</p>
          ...
          <div className="appointments-grid">
            {upcoming.map((app) => (
              <div key={app.id} className="appointment-card">
                ...
                <p><strong>Date:</strong> {new Date(app.appointment_time).toLocaleDateString()}</p>
                <p><strong>Time:</strong> {new Date(app.appointment_time).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p>
                <p><strong>Therapist:</strong> {app.therapist_name}</p>
                <p><strong>Type:</strong> {app.appointment_type === "virtual" ? "Online" : "In-Person"}</p>
                {app.reasonForMeeting && (
                  <p className="reason"><strong>Reason:</strong> {app.reasonForMeeting.slice(0, 50)}{app.reasonForMeeting.length > 50 ? "..." : ""}</p>
                )}
              </div>
            ))}
          </div>
        ...
        {childrenData.length === 0 ? (
          ...
          <div className="children-grid">
            {childrenData.map((child) => {
              const nextAppointment = upcoming.find((app) => app.child_name === `${child.first_name} ${child.last_name}`);
              return (
                <div key={child.id} className="child-card-summary">
                  <h4>{child.first_name} {child.last_name}</h4>
                  <p><strong>DOB:</strong> {child.date_of_birth}</p>
                  <p><strong>Next:</strong> {nextAppointment ? new Date(nextAppointment.appointment_time).toLocaleString([], { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" }) : "No upcoming appointments"}</p>
                </div>
              );
            })}
          </div>
      ...
      <section className="tips-section">
        ...
        <ul className="tips-list">
          <li><strong>Prepare:</strong> Test your video setup for online sessions.</li>
          <li><strong>Plan Ahead:</strong> Book early to secure your preferred therapist.</li>
          <li><strong>Stay Updated:</strong> Check this dashboard for your latest schedule.</li>
        </ul>
      </section>
    </div>
  );
            ...
            {availableSlots.length === 0 ? (
              ...
              <div className="therapist-row">
                {availableSlots.map((group, idx) => (
                  <div
                    key={idx}
                    className={`therapist-row-item selectable ${selectedTherapist && selectedTherapist.therapist_name === group.therapist_name ? "selected" : ""}`}
                    onClick={() => {
                    ...
                    <div className="timeslot-category">
                      ...
                      <div className="timeslot-list">
                        {categorizedSlots.Morning.map((slot) => {
                          // Disable the slot if the booking date is today and its time is already past.
                          const [startHour, startMinute, startSecond] = slot.start_time.split(":").map(Number);
                          ...
                          return (
                            <button
                              key={slot.id}
                              className={`time-slot-btn ${selectedSlot && selectedSlot.id === slot.id ? "selected" : ""}`}
                              onClick={() => !isPast && setSelectedSlot(slot)}
                              disabled={isPast}
                              style={isPast ? { backgroundColor: "#ccc", cursor: "not-allowed" } : {}}
                            >
